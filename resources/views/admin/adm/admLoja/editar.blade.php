...
@section('conteudo')
    ...
    @endif
    <form action="{{ route('loja.update') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label for="nome_loja" class="form-label">Nome da loja</label>
            <input type="text" name="nome_loja" class="form-control" id="nome_loja"
                placeholder="Titulo da categoria" value="{{ old('nome_loja', $loja->nome_loja) }}" maxlength="20">
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="regras_cupons" class="form-control" id="regras_cupons" placeholder="Digite as regras de seus cupons"
                rows="4">{{ old('regras_cupons', $loja->regras_cupons) }}</textarea>
        </div>
