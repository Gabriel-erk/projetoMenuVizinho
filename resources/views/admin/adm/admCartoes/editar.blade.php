...
@section('conteudo')
    ...
    @endif
    <form action="{{ route('pagamentos.atualizarPagamento',['id' => $cartao->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="number" name="user_id" class="form-control" id="user_id" placeholder="ID do usuário"
                value="{{ old('user_id', $cartao->user_id) }}" maxlength="4">
        </div>

        <div class="mb-3">
            <label for="numero_cartao" class="form-label">Número do cartão</label>
            <input type="text" name="numero_cartao" class="form-control" id="numero_cartao"
                placeholder="Número do cartão" value="{{ old('numero_cartao', $cartao->numero_cartao) }}">
        </div>

        <div class="mb-3">
            <label for="cvv" class="form-label">CVV</label>
            <input type="number" name="cvv" class="form-control" id="cvv"
                value="{{ old('numero_cartao', $cartao->cvv) }}" maxlength="3" placeholder="CVV">
        </div>

        <div class="mb-3">
            <label for="data_vencimento" class="form-label">Data de vencimento</label>
            <input type="date" name="data_vencimento" class="form-control" id="data_vencimento"
                value="{{ old('numero_cartao', $cartao->data_vencimento) }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nome_titular" class="form-control" id="nome_titular"
                value="{{ old('numero_cartao', $cartao->nome_titular) }}" placeholder="Nome do titular">
        </div>

        <div class="mb-3">
            <label for="cpf" class="form-label">CPF</label>
            <input type="text" name="cpf" class="form-control" id="cpf"
                value="{{ old('numero_cartao', $cartao->numero_cartao) }}" maxlength="14" placeholder="CPF">
        </div>
